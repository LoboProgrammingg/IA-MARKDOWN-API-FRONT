...
def generate_ai_image(prompt: str):
    ...
    if not API_KEY:
        print(
            'AVISO: A chave de API do Google AI (GOOGLE_AI_API_KEY) não está configurada em settings.py.'
        )
        return None
    ...
    try:
        ...
        if 'predictions' in result and result['predictions']:
            ...
            print(
                f'ERRO: A resposta da API não continha uma imagem válida. Resposta: {result}'
            )
            return None

    except requests.exceptions.RequestException as e:
        print(
            f'ERRO: Falha na comunicação com a API de geração de imagem: {e}'
        )
        return None
    except (KeyError, IndexError) as e:
        print(
            f'ERRO: A estrutura da resposta da API mudou ou está inesperada: {e}'
        )
        return None
